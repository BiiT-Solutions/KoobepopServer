...
public class CredentialsRegistry {
	...
	public static synchronized void registerCredential(IUser<Long> credential, VaadinSession session) {
		KoobepopLogger.debug(CredentialsRegistry.class.getName(), "User '" + credential.getEmailAddress() + "' tries to register in session '"
				+ getSessionId(session) + "'");
		if (sessionByCredential.containsKey(credential)) {
			VaadinSession sessionToClose = sessionByCredential.get(credential);
			if (Objects.equals(session, sessionToClose)) {
				KoobepopLogger.debug(CredentialsRegistry.class.getName(), "User '" + credential.getEmailAddress()
						+ "' was already registered in current session, no changes to session registry. ");
				return;
			}
			KoobepopLogger.debug(CredentialsRegistry.class.getName(), "User '" + credential.getEmailAddress()
	...
	public static synchronized void removeSession(VaadinSession session) {
